...
`denied-peer-ip` 列表来源于 [这里](https://www.enablesecurity.com/blog/cve-2020-26262-bypass-of-coturns-access-control-protection/).
...
sudo podman run --rm ghcr.io/authelia/authelia:4.39.20 \
  authelia crypto hash generate pbkdf2 --variant sha512 --password "$HEADSCALE_OIDC_SECRET"
```

修改 `/opt/authelia/config/configuration.yml` 添加 client

```
    clients:
      - client_id: headscale
        client_name: Headscale
        client_secret: "填写上面的 SECRET"
        authorization_policy: one_factor
        redirect_uris:
...
```bash
install -d -m 755 /opt/headscale
echo '[]' > /opt/headscale/config/extra_records.json
...
oidc:
  ...
  client_id: headscale
  client_secret_path: "/etc/headscale/headscale_oidc_secret_plain"
...
```bash
mkdir -p /opt/headplane/secrets /opt/headplane/data

podman exec -it headscale headscale apikeys create --expiration 365d \
  > /opt/headplane/secrets/headscale_api_key

openssl rand -hex 16 > /opt/headplane/secrets/cookie_secret
ln -s /opt/headscale/config/headscale_oidc_secret_plain /opt/headplane/secrets/headplane_oidc_secret_plain

chmod 600 /opt/headplane/secrets/*
```

写 `/etc/containers/systemd/headplane.container`
...
[Container]
Image=ghcr.io/tale/headplane:0.7.0-beta.4
ContainerName=headplane

# 因为你的 Headscale listen_addr 是 127.0.0.1:8070
Network=host

Volume=/opt/headplane/config.yaml:/etc/headplane/config.yaml:ro
...
headscale:
  # 因为下面 Quadlet 用 Network=host，所以这里可以直接连宿主机上的 Headscale
  ...
  # 你的 Headscale 如果是较新/开发版，建议先 false，避免 Headplane 不认识新字段直接拒绝工作
  config_strict: false
